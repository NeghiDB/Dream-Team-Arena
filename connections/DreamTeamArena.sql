...
CREATE TABLE IF NOT EXISTS `player` (
  ...
  `PlayerPoint` varchar(255) NOT NULL,
  `TotalPlayerPoint` varchar(255) NOT NULL,
...
DROP VIEW IF EXISTS `players`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `players`  AS SELECT `player`.`PlayerID` AS `playerid`, concat(`player`.`LastName`,' ',`player`.`FirstName`) AS `FullName`, `player`.`Position` AS `Position`, `player`.`Club` AS `Club`, `player`.`TotalPlayerPoint` AS `TotalPlayerPoint` FROM `player` ;
COMMIT;
